...
//#include <stack/ble/gap_event.h>

#include "spp.h"


extern int	module_uart_send_flg;
...
int bls_uart_handler (u8 *p, int n)
{
	...
	{
		pEvt->param[0] = blt_set_adv_addrtype(cmdPara); //bls_ll_setAdvType
	}
	...
	else if (spp_cmd == SPP_CMD_GET_CONN_PARA)
	{
		...
		if(interval){
			...
			pEvt->param[0] = HCI_ERR_CONN_NOT_ESTABLISH; ///LL_ERR_CONNECTION_NOT_ESTABLISH;
		}
	...
	else if (spp_cmd == SPP_CMD_SEND_NOTIFY_DATA)
	{
		...
		{
			pEvt->param[0] = bls_att_pushNotifyData( cmdPara[0] | (cmdPara[1]<<8), cmdPara + 2,  pCmd->paramLen - 2);//spp_cmd_t
		}
	}
...
int spp_send_data (u32 header, spp_event_t * pEvt)
{
	...
	if(!module_uart_send_flg){ //UART idle, new data is sent
		GPIO_WAKEUP_MCU_HIGH;  //Notify MCU that there is data here
		module_wakeup_mcu_tick = clock_time() | 1;
		module_uart_send_flg = 1;
	}
#endif
...
int tx_to_uart_cb (void)
{
	...
	if (p && !uart_tx_is_busy ())
	{
		...
		if(module_wakeup_mcu_tick){
			while( !clock_time_exceed(module_wakeup_mcu_tick, 100) );
		}
#endif



		if (uart_dma_send((u16 *)(&T_txdata_buf))) ///just send the data in DMA module.
		{
